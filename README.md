...
App ID 480 — общая тестовая среда Valve, а не личное пространство проекта и не автоматически разрешённая схема публичного релиза. Перед публикацией бинарника издатель должен выполнить условия Steamworks SDK и получить подтверждение Valve для выбранного способа использования либо перейти на согласованный собственный App ID. Транспорт также использует устаревший `ISteamNetworking`; в будущем его следует перенести на Steam Networking Sockets.
# e-steam
